...
/// <summary>
/// Toggles "launch dockdev at sign-in", by whichever mechanism this build's packaging supports.
/// <para>
...
public static class StartupService
{
    ...
    /// </summary>
    private const string TaskId = "dockdevStartupTask";

    private const string RunKey = @"Software\Microsoft\Windows\CurrentVersion\Run";
    private const string ValueName = "dockdev";

    /// <summary>What the startup entry ended up as after a request to change it.</summary>
    public enum StartupState
    {
        /// <summary>dockdev will not start at sign-in.</summary>
        Disabled,

        /// <summary>dockdev will start at sign-in.</summary>
        Enabled,

        /// <summary>The user turned dockdev off in Task Manager / Settings ▸ Startup apps. Only the
    ...
    /// <summary>True when dockdev is currently set to start at sign-in.</summary>
    ...
    private static void SetRunKey(bool enabled)
    {
        try
        {
            ...
            if (enabled)
            {
                var exe = Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "dockdev.exe");
