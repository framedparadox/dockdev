...
/// <summary>
/// The opt-in update check: asks GitHub for the latest release of <c>dockdev</c> and reports
/// whether it is newer than this build.
/// <para>
/// <b>Nothing here runs unless the user turns it on</b> (see <c>DockConfig.CheckForUpdates</c>,
/// which defaults to false). That matters more than the feature does: dockdev's privacy promise is
...
/// kind beyond the HTTP request itself — and it never downloads or installs anything. dockdev is a
...
public static class UpdateService
{
    private const string LatestReleaseApi =
        "https://api.github.com/repos/framedparadox/dockdev/releases/latest";
    ...
    public static async Task<ReleaseInfo?> CheckAsync(DockConfig config, Version current, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "dockdev-Dock-UpdateCheck");
    ...
    /// <para>
    /// <c>html_url</c> is remote, attacker-influenceable JSON, and the only thing dockdev does with it
