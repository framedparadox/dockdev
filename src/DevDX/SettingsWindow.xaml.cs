...
/// <para>
/// It talks to the <see cref="DevDxManager"/> rather than to the dock directly: most of what it
/// changes is app-wide.
/// </para>
/// </summary>
public sealed partial class SettingsWindow : Window
{
    ...
    public SettingsWindow(DevDxManager manager)
    {
        _manager = manager;
        InitializeComponent();
    ...
    private async void Export_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            var path = await FilePickers.PickSaveFileAsync(_hwnd, "devdx", ".json", "JSON");
            if (path is null)
                return;
    ...
    private Border BuildDockCard()
    {
        ...
        var displays = DevDxManager.Displays;
        if (displays.Count > 1)
        {
            ...
            monitors.SelectedIndex = DevDxManager.DisplayIndexOf(profile);
    ...
    /// plus an accent border — while a tool that is off reads as disabled: dimmed, but still
    /// legible and still switchable.</summary>
    private Border BuildToolCard(DockWindow dock, ToolDefinition tool)
    {
        ...
        {
            Background = (Brush)Application.Current.Resources["CardBackgroundFillColorDefaultBrush"],
            // An accent rim on the active card. Together with the bar on its leading edge this is
            // the same "selected" language ListView and NavigationView use.
            BorderBrush = (Brush)Application.Current.Resources[
                active ? "AccentFillColorDefaultBrush" : "CardStrokeColorDefaultBrush"],
            BorderThickness = new Thickness(1),
            CornerRadius = new CornerRadius(8),
