...
/// <summary>The two networked capabilities DevDX has, both off by default and both gated through
...
public sealed class NetworkSettings
{
    ...
    /// this is the <b>only</b> thing that makes DevDX talk to the network in v1 (Currency
...
public sealed class DockConfig
{
    ...
    /// <summary>
    /// The single dock strip DevDX shows. DevDX runs exactly one — there is no "add a second dock"
    ...
    /// a DevDX config would import as a plausible-looking empty one.
    /// </summary>
    ...
    /// <summary>Start DevDX automatically when the user signs in (per-user Run key).</summary>
    ...
    /// <summary>
    /// The language DevDX's own UI uses, as a BCP-47 code from <c>Loc.Available</c> (e.g.
