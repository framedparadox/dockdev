...
/// <para>
/// DevDX ships both ways from one codebase, and a handful of behaviors genuinely have to differ
...
///   DevDX pointing users at a GitHub release would be both redundant and a distribution route
///   outside the Store. The whole feature is therefore hidden when packaged — see
///   <see cref="DevDxManager.UpdateChecksSupported"/>.</description></item>
/// </list>
/// <para>
...
{
    /// <summary>True when DevDX is running from its MSIX package (the Store build).</summary>
    ...
    private static bool Resolve()
    {
        ...
        {
            // The API is missing only on pre-Windows-8 systems, which DevDX doesn't target — but
