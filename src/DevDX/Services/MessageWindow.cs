...
{
    // Unique per process: two copies of DevDX in one session are already prevented by the instance mutex,
    // but a stale class registration from a previous AppDomain would make RegisterClassEx fail.
    private static readonly string ClassName = "DevDX.MessageWindow." + Environment.ProcessId;
    ...
    public MessageWindow()
    {
        ...
        Handle = NativeMethods.CreateWindowEx(
            (int)NativeMethods.WS_EX_TOOLWINDOW, ClassName, "DevDX", (uint)NativeMethods.WS_POPUP,
