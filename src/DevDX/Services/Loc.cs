...
/// <summary>
/// DevDX's string table. Translations live as embedded JSON dictionaries under <c>Strings\</c>
...
/// <para>
/// Deliberately not the WinUI resource (.resw/PRI) pipeline: DevDX ships unpackaged, where the
...
{
    /// <summary>Every language DevDX ships, English first, then alphabetical by code.</summary>
    ...
    /// <summary>
    /// Maps a culture onto the closest language DevDX ships: exact code, then the Chinese script
    ...
    /// arguments DevDX substitutes are version numbers and exception messages, not user-facing
    ...
    {
        var name = $"DevDX.Strings.{code}.json";
        try
        {
