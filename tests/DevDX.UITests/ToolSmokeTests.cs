...
public class ToolSmokeTests(DevDxSession session)
{
    // ---- Every tool opens ---------------------------------------------------
    ...
    public static TheoryData<string> AllTools()
    {
        ...
        foreach (var kind in DevDxSession.ToolKinds)
            data.Add(kind);
        return data;
    }
    ...
    public void OpensFromTheDockAndCloses(string kind)
    {
        ...
        try
        {
            Assert.True(DevDxSession.RetryUntil(() => window.FindAllDescendants().Length > 3),
    ...
    /// per catalog entry — no reference to DevDX needed, and it fails the day someone adds a tool
    /// without adding a row.
    /// </summary>
    [UIFact]
    public void EveryCatalogToolHasARow()
    {
        var stringTable = Path.Combine(RepositoryRoot(), "src", "DevDX", "Strings", "en.json");
        ...
        var covered = DevDxSession.ToolKinds.Select(NameKeyFor).ToHashSet(StringComparer.OrdinalIgnoreCase);

        var missing = keys.Where(k => !covered.Contains(k)).Order().ToList();
        Assert.True(missing.Count == 0,
            "These catalog tools have no row in DevDxSession.ToolKinds: " + string.Join(", ", missing));
    }
    ...
    public void TransformsTheInput(TransformCase testCase)
    {
        ...
        try
        {
            ...
            var command = DevDxSession.Retry(() => window.FindFirstDescendant(
                cf => cf.ByAutomationId(testCase.CommandId)))
                ?? throw new InvalidOperationException(
                    $"'{testCase.Kind}' has no command '{testCase.CommandId}'.");
            DevDxSession.Press(command);

            Assert.True(
                DevDxSession.RetryUntil(() => VisibleText(window).Contains(testCase.ExpectedFragment, StringComparison.Ordinal)),
    ...
    /// </summary>
    internal static void CloseDiscardingChanges(DevDxSession session, Window window)
    {
        session.CloseAndWait(window);

        var discard = DevDxSession.Retry(
            () => window.FindFirstDescendant(cf => cf.ByControlType(ControlType.Button)
                    .And(cf.ByName("Discard"))),
            TimeSpan.FromSeconds(3));

        if (discard is not null)
            DevDxSession.Press(discard);
