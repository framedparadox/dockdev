...
public class InAppLinkTests
{
    ...
    private static readonly Regex RepositoryFileLink = new(
        @"https://github\.com/framedparadox/dockdev/blob/(?<ref>[^/""\s]+)/(?<path>[^""\s<>]+)",
    ...
    private static List<(string File, string Url, string Ref, string Path)> RepositoryFileLinks()
    {
        ...
        var source = Path.Combine(RepositoryRoot, "src", "dockdev");
    ...
    /// directory that contains <c>src/dockdev</c>.</summary>
    private static string RepositoryRoot
    {
        get
        {
            ...
            {
                if (Directory.Exists(Path.Combine(dir.FullName, "src", "dockdev")))
                    return dir.FullName;
                dir = dir.Parent;
            }
