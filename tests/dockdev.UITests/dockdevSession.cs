...
/// <summary>
/// One running dockdev, shared by every test in the collection.
/// <para>
...
/// </para>
/// <para>
/// The session is pointed at a throwaway data directory via <c>DOCKDEV_DATA_DIR</c> and seeds a
...
/// directory, does not collide with a dockdev the developer already has running.
/// </para>
/// </summary>
public sealed class dockdevSession : IDisposable
{
    ...
    /// <para>
    /// Caching it looks obviously right and is obviously wrong: dockdev rebuilds windows in response
    ...
    public dockdevSession()
    {
        ...
        info.EnvironmentVariables["DOCKDEV_DATA_DIR"] = DataDirectory;
    ...
    /// children by <c>ControlType.Window</c>. dockdev's windows are borderless and extend their
    ...
    /// not supported" on dockdev's borderless windows — the same reason those windows report as Panes
    ...
    /// root Grid, not the window, and every window in the app answers to the title "dockdev".
    /// <para>
    ...
    private Window? FindDock() =>
        TopLevelWindows()
            .FirstOrDefault(w => w.FindFirstDescendant(cf => cf.ByAutomationId("dockdevSettingsButton")) is not null);
    ...
    /// <summary>
    /// The dockdev to drive. <c>DOCKDEV_EXE</c> wins; otherwise the Release build in the source tree,
    ...
    {
        var configured = Environment.GetEnvironmentVariable("DOCKDEV_EXE");
        if (!string.IsNullOrWhiteSpace(configured))
        {
            if (!File.Exists(configured))
                throw new FileNotFoundException($"DOCKDEV_EXE points at a file that isn't there: {configured}");
            return configured;
        }
        ...
        string[] candidates =
        [
            Path.Combine(root, "src", "dockdev", "bin", "x64", "Release", Tfm, "win-x64", "dockdev.exe"),
            Path.Combine(root, "src", "dockdev", "bin", "Release", Tfm, "win-x64", "dockdev.exe"),
            Path.Combine(root, "src", "dockdev", "bin", "Debug", Tfm, "win-x64", "dockdev.exe"),
        ];

        return candidates.FirstOrDefault(File.Exists)
            ?? throw new FileNotFoundException(
                "No dockdev.exe found. Build the app first, or set DOCKDEV_EXE. Looked in:" +
    ...
    /// deliberately has no reference to dockdev. The shape is small and the app's loader tolerates
...
/// putting all the UI tests in one collection is what keeps it to a single dockdev rather than one
/// per test class — and, just as importantly, stops two of them driving the desktop at once.
/// </summary>
[CollectionDefinition(Name)]
public sealed class dockdevCollection : ICollectionFixture<dockdevSession>
{
    public const string Name = "dockdev UI";
}

/// <summary>
/// Skips the whole suite unless <c>DOCKDEV_UITESTS</c> is set. These need a real, visible, logged-in
...
public sealed class UIFactAttribute : FactAttribute
{
    ...
    {
        if (!Enabled)
            Skip = "Set DOCKDEV_UITESTS=1 to run the UI tests (they need an interactive desktop).";
    }

    public static bool Enabled =>
        !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DOCKDEV_UITESTS"));
}
