...
**DevDX** is a WinUI 3 floating dock of developer tools. The *shell* is a single strip of
...
CSV into a random website is a data-handling incident waiting to happen. DevDX is the same tools,
...
- No folder fly-outs / stacks (tied to a file-system concept DevDX does not have).
...
|---|---|---|
| 1 | Name **DevDX**, new repo, MIT license | Find-and-replace before scaffolding |
...
```mermaid
flowchart TB
    subgraph Process["DevDX.exe — single process, single instance (named mutex)"]
        App["App.xaml.cs<br/>entry point · mutex · Loc.Initialize"]
        Manager["DevDxManager<br/>config · tray · global hotkeys · window lists"]
...
| Architectures | `x64` + `ARM64` | `Platform`→RID normalization in `DevDX.csproj`: `-p:Platform=ARM64` builds natively for Windows-on-ARM |
...
Notably absent is `System.Drawing.Common`: it is what shell-icon extraction needs, and DevDX has
...
dev-dx/
  ...
  docs/
    devdx-design-document.md          (this file)
  ...
  src/DevDX/
    ...
    DevDxManager.cs                   Config · tray · hotkeys · window lists · ToolWindowManager
    ...
    Assets/DevDX.ico · Images/ · Package.appxmanifest · app.manifest · DevDX.csproj
  tests/
    DevDX.Tests/                      xUnit — every service, plus fixture corpora
    DevDX.UITests/                    FlaUI — table-driven smoke, one row per tool
...
- **Cache.** `%AppData%\DevDX\rates-cache.json` with a fetch timestamp. Offline shows
...
`%AppData%\DevDX\config.json`, guarded by a single-instance mutex
(`Local\DevDX.SingleInstance.v1.<sha256-of-data-dir>`).
...
DevDX never calls `ShellExecute` on an arbitrary target and never makes an unrequested network
...
**`tests/DevDX.Tests` (xUnit, runs anywhere):**
...
Driven by `scripts/run-ui-tests.ps1`, opted into with `DEVDX_UITESTS`/`DEVDX_EXE`, pointed at a
throwaway data directory via `DEVDX_DATA_DIR`, and excluded from the main `.slnx`.

---

## 25. Build, packaging, distribution & CI

- **Build:** `dotnet build src/DevDX/DevDX.csproj -p:Platform=x64` (and `ARM64`).
...
| Concern a launcher has | What DevDX does instead |
|---|---|
...
| # | Scope | Exit criteria |
|---|---|---|
| **M0** | This document approved; scaffold `DevDX.slnx` + csproj | `dotnet build` clean |
...
```jsonc
{
  ...
  "Currency.ConsentBody": "DevDX can download reference rates from the European Central Bank. No personal data is sent. You can also enter rates manually.",
